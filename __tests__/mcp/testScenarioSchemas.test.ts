...
describe("structured Test Scenario MCP schemas", () => {
  it("keeps list and evidence pagination defaults independent", () => {
    expect(listTestScenariosSchema.parse({ projectId })).toEqual({ projectId, page: 1, limit: 30 });
  ...
  it("accepts structured update fields and nullable clearing", () => {
    expect(updateTestScenarioSchema.parse({ scenarioId, projectId, notes: null })).toMatchObject({ notes: null });
    expect(updateTestScenarioSchema.parse({ scenarioId, projectId, objective: "Objective" })).toMatchObject({ objective: "Objective" });
  ...
  it("rejects invalid identifiers, pagination, and obsolete creator inputs", () => {
    expect(listTestScenariosSchema.safeParse({ projectId: "bad" }).success).toBe(false);
    expect(getTestScenarioSchema.safeParse({ scenarioId, projectId, resultLimit: 101 }).success).toBe(false);
    expect(deleteTestScenarioSchema.safeParse({ scenarioId, projectId, createdById: scenarioId }).success).toBe(false);
  });
});
