---
last_mapped: 2026-08-05T13:03:51Z
total_files: 500
total_tokens: 395373
---
...
> Auto-generated by Cartodex, a Codex port inspired by Cartographer. Last mapped: 2026-08-05T13:03:51Z
...
Core domain data is project-scoped and shared across active authenticated users. `Project.ownerId` records attribution; it is not a tenant, visibility, or authorization boundary. `Project` owns executions, specs, issues, upload API keys, and daily metrics. `Execution` plus `Spec` produce `Result`; failed results can have `ResultError`; issues and result errors connect through `Assumption`. `DailyExecutionMetric` is a derived aggregate table refreshed from result data. Future organization-based isolation would be a separate breaking capability with its own membership and migration model.
...
| `src/routes` | Versioned REST route modules plus OpenAPI and MCP mount. | 3592 |
| `src/controllers` | HTTP request/response adapters and validation glue. | 17153 |
| `src/services` | Business logic, report ingestion, auth, dashboards, prompts, PDF/export workflows. | 37245 |
| `src/models` | Prisma persistence wrappers and manual delete ordering. | 11817 |
| `src/mcp` | MCP Streamable HTTP server, tools, schemas, middleware, and assistant prompts. | 16835 |
| `src/handlers` | Thin MCP-to-service adapters. | 1863 |
| `src/lib` | Shared helpers, OpenAPI generator modules, logging, parsing, token utilities. | 40282 |
| `src/lib/openapi` | Manually assembled OpenAPI 3.1 registrars. | 30720 |
| `src/middleware` | JWT auth, upload API key auth, multipart upload, PDF export validation, error handler. | 1595 |
...
| `src/types` | Shared API, Prisma-shaped, MCP, CTRF, dashboard, skills, and test types. | 5400 |
| `prisma` | PostgreSQL schema, migrations, seed and migration helpers. | 14739 |
| `__tests__` | Jest unit/integration coverage for routes, controllers, services, models, middleware, lib. | 68270 |
| `__prompts-tests__` | Prompt evaluation runners, templates, dataset generators, smoke/regression tests. | 10585 |
| `docs` | Human-facing API, MCP, Docker, deploy, auth, dashboard, release, and research docs. | 35275 |
| `openspec` | Permanent specs plus active and archived OpenSpec changes. | 45210 |
| `postman` | REST/MCP Postman collections, environment, and usage docs. | 18729 |
| `scripts` | License header and new-file automation. | 1744 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `package.json` | Scripts, dependencies, package entrypoint. | 1205 |
...
| `src/lib/openapi/index.ts` | Generates the OpenAPI 3.1 document from per-domain registrars. | 1027 |
| `src/config/environment.ts` | Runtime env config, app version, Cognito/LangSmith flags. | 887 |
| `src/middleware/fileUploadMiddleware.ts` | In-memory multipart upload handling. | 450 |
| `src/middleware/validatePdfExport.ts` | PDF export request validation and `res.locals.exportParams` handoff. | 298 |
| `src/controllers/jsonReportController.ts` | JSON report upload, analysis, stats refresh orchestration. | 1951 |
| `src/controllers/resultController.ts` | Result read/update/delete/export handlers. | 2060 |
| `src/controllers/userController.ts` | Signup/login/profile/integrations/MCP token handlers. | 2306 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `prisma/schema.prisma` | Canonical PostgreSQL schema with UUID IDs and JSON payload fields. | 1950 |
| `src/services/jsonReportService.ts` | Playwright-like report ingestion and persistence. | 2219 |
| `src/services/ctrfService.ts` | CTRF transformation and ingestion path. | 1761 |
| `src/services/testAnalysisService.ts` | AI test-result analysis orchestration. | 1018 |
| `src/services/dashboardService.ts` | Daily metric refresh and dashboard reads. | 2958 |
| `src/services/resultService.ts` | Result querying, analysis feedback, deletion, JSONL export. | 2577 |
| `src/services/skillArtifactService.ts` | Skill catalog, frontmatter parsing, downloads, zip archives. | 1861 |
| `src/models/resultModel.ts` | Result Prisma queries and nested filtering. | 4598 |
...
| `src/handlers/mcpResultHandler.ts` | Result MCP handler adapter. | 529 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `docs/API_DOCUMENTATION.md` | REST/OpenAPI and MCP contract guide. | 2626 |
...
| `docs/DEPLOY_ECS.md` | ECS deployment and migration caveats. | 1912 |
| `docs/RELEASE.md` | Release/version/tag workflow. | 473 |
| `postman` | API collections and shared environment. | 18729 |
| `openspec/specs` | Permanent accepted specs. | 6988 |
...
| File | Purpose | Tokens |
| --- | --- | ---: |
| `jest.config.ts` | Default Jest config, excluding prompt tests. | 178 |
| `jest.prompts.config.ts` | Separate prompt-test Jest projects. | 153 |
| `eslint.config.js` | ESLint rules and TS parser setup. | 624 |
...
| `README.md` | Setup, scripts, API surface, documentation index. | 1453 |
...
- Active authenticated users share project reads and mutations. `Project.ownerId` is attribution metadata only; project-scoped lookups use `projectId`, not the owner, as their resource boundary.
...
- Interactive workspace access is shared, but upload API keys remain scoped machine credentials: the signed key ID, project ID, owner ID, and payload must match an active stored record.
