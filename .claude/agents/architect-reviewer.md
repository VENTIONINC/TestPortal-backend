...
- `src/services/*` - Business logic purity
- `src/handlers/*` - MCP-specific logic
...
Before approving any changes, ensure:
1. `npm run type-check` passes
2. `npm run lint` has no errors
3. `npm run build` succeeds
4. Architectural patterns maintained
5. No regression in error handling

Always consider both REST API and MCP interfaces when reviewing changes.