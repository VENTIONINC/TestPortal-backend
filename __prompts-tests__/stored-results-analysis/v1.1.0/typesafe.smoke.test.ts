...
describe("TypeSafe Evaluation - Smoke Tests (jev-1.13.0)", () => {
  jest.setTimeout(120_000);

  it("classifies the smoke dataset", async () => {
    ...
    console.log(
      `TypeSafe smoke: ${(result.accuracy * 100).toFixed(1)}% accuracy, ${result.usage.inputTokens} input tokens, ${result.durationMs}ms`,
    );
