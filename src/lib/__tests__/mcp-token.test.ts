...
describe('MCP Token Generation and Validation', () => {
  ...
  describe('generateMcpToken', () => {
    it('should generate a valid MCP token with correct format', () => {
      ...
      expect(token).toMatch(/^mcp_\d+_\d+_[a-f0-9]{32}\.[a-f0-9]{64}$/);
    ...
    it('should generate different tokens for different users', () => {
      const token1 = generateMcpToken(12345, mockSecret);
      const token2 = generateMcpToken(67890, mockSecret);
      
      expect(token1).not.toBe(token2);
      expect(token1).toContain('mcp_12345_');
      expect(token2).toContain('mcp_67890_');
    });
  ...
  describe('validateMcpToken', () => {
    ...
    it('should return null for non-numeric user ID', () => {
      const malformedToken = 'mcp_abc_1234567890_randomhex.signature';
