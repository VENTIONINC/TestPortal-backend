...
/**
 * Generates a self-contained API key for external integrations
 * Format: api_{userId}_{timestamp}_{randomBytes}.{hmacSignature}
 */
export function generateApiKey(userId: number, secret: string): string {
  const timestamp = Date.now();
  const randomBytes = crypto.randomBytes(16).toString('hex');
  const payload = `api_${userId}_${timestamp}_${randomBytes}`;
 ...
 * Returns user info if valid, null if invalid
 */
export function validateApiKey(apiKey: string, secret: string): { userId: number; isValid: true } | null {
  try {
    const [payload, signature] = apiKey.split('.');
    if (!payload || !signature) return null;
    
    const [prefix, userIdStr, timestampStr, randomBytes] = payload.split('_');
    if (prefix !== 'api' || !userIdStr || !timestampStr || !randomBytes) return null;
