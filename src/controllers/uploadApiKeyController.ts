...
export const uploadApiKeyController = {
  ...
  async generateKey(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (!isOwner) {
        res.status(403).json({
          error: "You do not have permission to generate API keys for this project",
        });
        return;
      }

      logger.info(`Generating API key for project ${projectId} by user ${userId}`);

      const apiKeyData = await uploadApiKeyService.generateApiKey(projectId, userId);
