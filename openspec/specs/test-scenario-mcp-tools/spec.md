...
The system SHALL expose an `update-test-scenario` MCP tool requiring valid `scenarioId` and `projectId` inputs plus at least one of `title`, `contentMd`, or `details`. The tool SHALL call the established scenario update service so validation, trimming of supplied titles, exact Markdown preservation, last-write-wins behavior, immutable metadata, and project scoping remain equivalent to REST PATCH behavior. The successful response SHALL contain the complete persisted Test Scenario.
...
- **WHEN** the client supplies no editable field, a blank title, an empty or non-string Markdown value, or an unsupported input field
