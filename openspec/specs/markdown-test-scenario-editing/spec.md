# markdown-test-scenario-editing Specification

## Purpose
Define strict project-scoped partial scenario editing while preserving identity and linked execution evidence.

## Requirements

### Requirement: Authenticated clients can partially update a Test Scenario
The system SHALL expose `PATCH /api/v2/test-scenarios/{scenarioId}` to authenticated clients and SHALL require a valid `projectId` query parameter. The request SHALL contain at least one of `title`, `contentMd`, or `details`, and the system SHALL update all supplied editable fields atomically while preserving omitted fields.
...
The update body SHALL contain at least one editable field. Supplied titles SHALL be strings that remain non-empty after trimming, and supplied `contentMd` values SHALL be non-empty strings. Details SHALL follow the nullable clearing and trimming rules in test-scenario-summaries. The system SHALL reject null title/contentMd, malformed, unknown, and read-only fields rather than silently ignoring them.
...
- **WHEN** an authenticated client submits null for title/contentMd or a non-string non-null value for an editable field
- **THEN** the system returns HTTP 400 with an `{ "error": string }` response
- **AND** leaves the scenario unchanged
